...
\begin{figure}[h]
    ...
    \end{subfigure}
	\caption{Discontinuities in the values of $\uu[\pm 1]{\mathscr{E}}{\ell 1}$ for $m=1,2$ using incorrect implementation of the Leaver method. Real values of the eigenvalues are shown as dashed lines ($m=1,2,3$) of the same color.}
...
Modes in the region \eref{eq3:superradiance} have always $\uu[\pm 1]{Z}{\ell m}>0$, but decreases heavily in orders of magnitude for large $\ell$.
\begin{figure}[h]
	\centering
	\vspace{0.2cm}
	\begin{subfigure}[c]{0.6\textwidth}
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{Plot demonstrating error propagation for $\uu[\pm 1]{Z}{53}$ when computing the factor using both numerical solutions for the radial part of $\phi_0$ and $\phi_2$.}
	\label{fig4:plotZ53}
\end{figure}
...
Still we have huge difference in the order of magnitude of the amplification factor when comparing results from using only one solution and using both.
