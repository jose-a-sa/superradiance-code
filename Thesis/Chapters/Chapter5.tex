...
Proceeding with the sum, it appears that the partial wave sum is divergent near $\theta = 0$, since the value of increasing $f(0,0)$ seams to increase more with each contribution. (see \fref{fig5:sumF}).
\begin{figure}[h]
	\centering
	\vspace{0.2cm}
	...
	\label{fig5:sumF}
\end{figure}
This problem is due to the long-range effect of gravitational potential of BHs. Central potentials falling as $1/r$ (check \eref{eq3:asymptoticVeff}) do not have effect on the global amplitude of the have but the scattered wave has phase shifts in each of the modes coefficients, producing a divergence at $(\theta,\varphi)=(\theta_0,\varphi_0)$.
This problem was also studied in classical Coloumb scattering, where $f(\theta,0)$ has is known to diverge at $\theta=\theta_0$.
...
\end{align}
Assuming an incidence of $\theta_0=0$, summing the series leads to a similar result as the Rutherford elastic scattering in a Coulomb potential, $|f_\mathrm{N}(\theta,0)|^2 \sim 1/\sin^{4}(\theta/2) \sim 1/\theta^4$, which appears to explain the divergence at $\theta=0$. 
...
Secondly, the numerically computed phases using \eref{eq5:phaseShiftD} must converge for the the Newtonian phase shits \eref{eq5:phaseShiftN}, $\delta_\ell \to\delta_N$.
Results appear to indicate that these phases, like $\delta_N$, are independent of $m$.
Also they appear to have the same asymptotic form, apart from a constant value $\delta_0$. 
We attribute this difference to a ambiguity in the definition of the tortoise coordinate, given by
\begin{align}
...
\end{figure}
It is expect for this integration constant to be only dependent on $a$ and $M$, thus for constant $\bar{\omega}$ the value of $\delta_0$ should be dependent on $\mathscr{J}$.
We fit numerically the value of $\delta_0$ independently for each case.
From \fref{fig5:argZoutZin} we verify that these phases indeed share the same asymptotic behaviour for large values of $\ell$.
Effects of superradiance are predominant when $\ell$ is small and this is reflected in larger deviations of $\delta_\ell$ from $\delta_N$.
Decreasing the BH angular momentum, $\mathscr{J} \to 1$, quickly makes the values of $\delta_\ell$ come closer to $\delta_N$.
The respective regularized sums for these phases are plotted in \fref{fig5:fD}.
\begin{figure}[h]
	\centering
	\vspace{0.2cm}
	\begin{subfigure}[c]{0.49\textwidth}
	...
	\label{fig5:fD}
\end{figure}
Comparing with the previous \fref{fig5:sumF} it seams that the partial wave series for $|f_\mathrm{D}|^2$ is now converging, for values of $30<\ell_\mathrm{max}<50$.
