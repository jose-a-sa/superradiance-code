...
(INCLUDE WEYL DECOMPOSITION EF NEEDED)
...
Weyl scalar $\psi_2$ becomes invariant under boosts in the principal directions. (REFER THE TYPE III ROTATIONS EXPLICITLY)
...
Will be more profitable to study Eqs.~\eref{eq3:DDLLPhi0} and~\eref{eq3:DDLLPhi2} as a special case of the Teukolsky master equation which describes all the linearized perturbations around the Kerr geometry.
...
\begin{table}[h]
    ...
    \begin{tabular}{ | c | l | }
        ...
        \hline
    \end{tabular}
    \caption{Newman-Penrose fields that obey the Teukolsky master equation for different spin-weights}
...
In the spherical geometry, this corresponds to truncating the series at $r$, as $\gamma_p=0$ for any $p$. Since the the eigenvalue root depends on integer $r$, 
\begin{align}
    {}_{s}\mathscr{A}_{\ell m} = \ell(\ell+1) - s(s+1) + \mathscr{O}(a\omega)~,
    \label{eq3:evSWSH0th}
\end{align}
entails the discretization of the spectra, where we identified $\ell=r+k_{+}+k_{-}$. 
...
% From now on, we will be dropping the $(s, \ell, m)$ indices on the eigenvalues and on the radial and angular functions.
...
\begin{align}
    \begin{split}
        ...
        &\qquad + \left[ \frac{\bar{K}^2 - i s (2 x+\tau) \bar{K}}{x(x+\tau)} + 4 i s (1 + x)\bar{\omega} - {}_{s}\mathscr{F}_{\ell m} \right] {}_{s}R_{\ell m} = 0 ~,
    \end{split}
\end{align}
where we normalize $\bar{K} = K/r_{+} = \varpi + x(x+2)\bar{\omega}$. In this method, will be sufficient to use  this method we will use a spherical approximation for the harmonics eigenvalues, ${}_{s}\mathscr{F}_{\ell m} = \ell(\ell+1) - s(s+1) + \mathscr{O}(a\omega)$, for small enough frequencies.

Near horizon approximation corresponds to considering only small distances compared to the perturbations characteristic wavelength, $r-r_{+} \ll \omega^{-1}$ ($\bar{\omega} x \ll 1$).
Because superradiant scattering occurs when $\omega\sim\Omega_H$, therefore we also neglect terms $\mathscr{O}(\varpi x)$.
In this limit, $\bar{K}\simeq\varpi$.
The resultant equation remains singular at the horizons $x=0$ and $x=-\tau$. Thus, making the substitution ${}_{s}R_{\ell m}(x)\simeq x^\alpha (x+\tau)^\beta H(x)$, the function $H(x)$ is analytical if $\alpha+\beta=-s$ and also if $\alpha=-\tfrac{1}{2} s \pm \left(\tfrac{1}{2} s + i \varpi/\tau \right)$.
...
\end{align}
where ${}_{2}F_1(a,b,c; x)$ is the hypergeometric function.

Asymptotically, we consider $x\gg\tau$, where $\bar{K}\sim x^2 \bar{\omega}$. The resultant equation, 
% \begin{align}
...
\end{align}
The ingoing an outgoing wave coeficients can be related to integration coeficients $B,C$ by expanding the hypergeometric function in~\eref{eq3:RfarSolution} at infinity,
\begin{align}
    \begin{split}
...
Diferent Killing vectors have distinct currents, due to diferent possible projections of the stress-energy tensor.
...
In BL coordinates, the normal to the surface is the outgoing radial vector $\bm{n}=(\dd r)^\sharp \equiv ( g^{\mu\nu} \partial_\nu r) \partial_\mu$, while the other vectors form the tangent basis.
...
We are obiously interested in obtaining an expression relation the flow of energy at infinity and the Maxwell NP scalar. Thus, will be convininet describing the stress-energy tensor using symmetric tetrad combinations and NP scalars and their conjugates. Much like the Weyl and the Maxwell tensor, this composition is uniquelly defined by Eqs.~\eref{eq2:stressenergyEM} and~\eref{eq3:maxwellNPphi},
\begin{align}
    \begin{split}
...
In order to obtain the full conservation law we must find the absorved radiation by the black hole. We turn down to the horizon null hypersurface, which the normal vector $n$ in BL coordinates is the zero vector, due to $g^{rr}=0$.
...
The radial 3-surface element cannot be of the form of~\eqref{eq3:radial3SurfaceElement}, since the induced metric at the horizon is now singular, $\sqrt{\det{\bm{h}}}=\Delta \rho^2 \sin\theta = 0$.
...
\begin{align}
    \uu[\pm1]{Z}{\ell m} = \frac{\dd E_\mathrm{out} / \dd t}{\dd E_\mathrm{in} / \dd t} - 1 ~,
\end{align}
where we integrated off the polar angles.
The factor is defined as the overall gain/loss effect for each mode $(\omega,\ell,m)$, therefore it measures how much of the wave was \emph{globally} reflected ($\uu[\pm1]{Z}{\ell m}=0$), absorved ($\uu[\pm1]{Z}{\ell m}<0$) or amplified ($\uu[\pm1]{Z}{\ell m}>0$).
Remebering that $\phi_2 = \Phi_2/(2 \bar{\rho}^2) \sim {}_{-1}R_{\ell m}/(2 r^2)$, we compute
\begin{align}
...
In the large $r$ limit, this equation is simplified into $(\partial_r-i \omega)(\partial_r-i \omega) \,{}_{-1}R_{\ell m} \sim \mathscr{B} \,( {}_{+1}R_{\ell m} )$, and considering terms only up to $\mathscr{O}(r^{-1})$, we substitute in $A_\mathrm{in}(s=+1)$,
\begin{align}
...
Still, the amplification should not depend on the sign of spin-weight, \emph{i.e.} the amplification should be the same for all EM waves, the same holding true for GW perturbations~\cite{Rosa2016}.
...
\begin{align}
    ...
    \end{split}
\end{align}
where we appoximated the relative normalization constant $\mathscr{B} = \lambdabar +\mathscr{O}(a\omega) \simeq \ell(\ell+1)$.
The $D/C$ ratio has a non-trivial expression in terms of $\Gamma$ functions of non-integers arguments. The factorial property of $\Gamma(z)$ allows the approximation
\begin{align}
...
\end{align}
where $y\equiv - 2 i \varpi/\tau$. For EM perturbations the amplification factor yields
\begin{align}
...
\end{align}
which is valid for a region $\bar{\omega}\lesssim 1$. 
