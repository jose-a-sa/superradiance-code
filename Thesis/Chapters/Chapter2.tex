...
\section{Newman-Penrose equations}
...
Results as a natural introduction of spinor techniques into GR, after the choice of a null complex tetrad basis $(e_1, e_2, e_3, e_4)$. Penrose believed that the light-cone was the essential element of the spacetime, thus it was of importance to find null directions. The basis consisted in two real vectors, $l$ and $n$, and two complex conjugate vectors $m$ and $\bar{m}$. Besides satisfying
\begin{align}
...
\begin{align}
    g^{\mu\nu} = \eta^{ab} (e_a)^\mu (e_b)^\nu = l^{\mu} n^{\nu} + n^{\mu} l^{\nu} - m^{\mu} \bar{m}^{\nu} - \bar{m}^{\mu} m^{\nu} ~.
\end{align}
Additionally this vectors define new directional derivatives, $e_a=(e_a)^\mu \nabla_\mu$.
We will deviate shortly from standard notation~\cite{Teukolsky1972,Teukolsky1973,Teukolsky1974}, by redefining these derivatives as
\begin{align}
    ()
\end{align}
...
To study GWs, instead of perturbing the background metric, NP formalism provides a natural way of performing perturbations by modification of the tetrad, $l=l^B+l^P$, $n=n^B+n^P$, etc., and also the NP scalars, $\psi_a = \psi_a{}^B + \psi_a{}^P$, maintaining only first-order terms.
...
\begin{subequations}
    \begin{align}
        \phi_{2\rvert 1} &= \phi_{1\rvert 4} ~, \label{eq2:phi21phi14}\\
        \phi_{1\rvert 2} &= \phi_{2\rvert 3} ~, \label{eq2:phi12phi23}\\
        \phi_{1\rvert 1} &= \phi_{0\rvert 4} ~, \label{eq2:phi11phi04}\\
        \phi_{0\rvert 2} &= \phi_{1\rvert 3} ~. \label{eq2:phi02phi13}
    \end{align}
\end{subequations}
We may expand explicitly the left-hand side of~\eqref{eq2:phi21phi14},
\begin{align}
    \begin{split}
...
\begin{align}
    ...
    \label{eq2:phi21expand}
\end{align}
The operators $\mathbbl{D}$, $\bar{\bbdelta}$ and the spin coefficients $\varepsilon$, $\pi$, $\lambda$, $\tau$ as well as other NP definitions can be found in~\aref{AppendixNPSpinCoef}. If we repeat the same expansion for the other Maxwell equations, we gather the set
\begin{subequations}
    \begin{align}
        \mathbbl{D} \phi_2 - \bar{\bbdelta} \phi_1 &= -\lambda \phi_0 + 2 \pi \phi_1 + (\varrho - 2 \varepsilon) \phi_2 ~,\\
        \mathbbl{\Delta} \phi_1 - \bbdelta \phi_2 &= (2 \gamma - \mu) \phi_0 + 2 \varrho \phi_1 - \kappa \phi_2 ~,\\
        \mathbbl{D} \phi_1 - \bar{\bbdelta} \phi_0 &= (\pi - 2 \alpha) \phi_0 - 2 \tau \phi_1 + \sigma \phi_2 ~,\\
        \mathbbl{\Delta} \phi_0 - \bbdelta \phi_1 &= \nu \phi_0 - 2 \mu \phi_1 + (2\beta-\tau) \phi_2 ~.
    \end{align}
\end{subequations}
Chandrasekhar provided the 
