...
\end{align}
where $a$ is a parameter, $M$ is the Komar mass and $\rho^2 = r^2 + a^2 \cos^2\theta$. Naturally the time Killing vector is $\partial_v$ and $\partial_\chi$ is the axial field, entailing that $J = a M$.
...
As a result, in NP formalism the Weyl tensor can be represented by five complex scalars, usually chosen as~\cite{Chandrasekhar1998} 
\begin{align}
    \begin{split}
        \Psi_0 &= - C_{1313} = - C_{\mu\nu\rho\sigma} l^\mu m^\nu l^\rho m^\sigma ~,\qquad
        ~\Psi_1 = - C_{1213} = - C_{\mu\nu\rho\sigma} l^\mu n^\nu l^\rho m^\sigma ~,\\
        \Psi_2 &= - C_{1342} = - C_{\mu\nu\rho\sigma} l^\mu m^\nu \bar{m}^\rho n^\sigma ~,\qquad
        \Psi_3 = - C_{1242} = - C_{\mu\nu\rho\sigma} l^\mu n^\nu \bar{m}^\rho n^\sigma ~,\\
        \Psi_4 &= - C_{2424} = - C_{\mu\nu\rho\sigma} n^\mu \bar{m}^\nu n^\rho \bar{m}^\sigma ~.
    \end{split}
\end{align}
The complex conjugates can be obtained by doing the replacement $3\leftrightarrow 4$, by exchanging $m$ with $\bar{m}$ and vice-versa. Weyl tensor has a unique decomposition in therms of a linear combination of NP scalars and tensorial product of two-forms such as $l_{[\mu} n_{\nu]}$ and $m_{[\rho} \bar{m}_{\sigma]}$.

BHs solutions belong to the ``type D'' of Petrov's classification of spacetimes, which was a major restriction necessary to Kerr's discovery.
Solutions are classified by obtaining all null principal directions of the Weyl tensor. For BH spacetimes, $l$ and $n$ are chosen such that they are the two doubly-degenerate principal vectors, 
\begin{align}
    C_{\mu\nu\rho[\sigma} l_{\varepsilon]} l^\nu l^\rho = 0 ~, \qquad C_{\mu\nu\rho[\sigma} n_{\varepsilon]} n^\nu n^\rho = 0  ~.
\end{align}
In NP formalism terms, these spacetime are characterized by
\begin{align}
    \Psi_0=\Psi_1=0 ~,\qquad \Psi_3=\Psi_4=0 ~,
\end{align}
respectively.
Fields $l$ and $n$ are invariant under a subgroup of Lorentz transformations that rotate the tetrad frame, up to multiplication of scalar functions.
These rotations maintain the light-cone structure intact and are useful to change between ingoing and outgoing frames~\cite{Teukolsky1974}.

-~Introduce  Kinnersly tetrat (cite)

-~Confirm null directions and compute $\Psi_2$.

-~Talk about the perturbations in NP formalism

\subsection{Maxwell equations}

-~Project Mawell equations and derive everyting (use notes)
