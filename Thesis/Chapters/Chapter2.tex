...
On the other hand, as we approach the singularity on $z=0$ and $x^2+y^2 = a^2$, rotation effects deform the surfaces into oblate spheroids (for the strict inequality, $\theta\ne\pi/2$).
...
\begin{align}
    \Omega_H = \frac{a}{2 M r_+} = \frac{a}{2 M \left(M+\sqrt{M^2-a^2}\right)} ~.
...
\begin{align}
    ...
    \label{eq2:KillingXi}
\end{align}
is also a null vector normal to the event horizon, $\xi^\flat=\xi_\alpha \dd x^\alpha \propto \dd r$ and $\xi^2 = 0$ at $r=r_+$, which can be written in BL coordinates as $\xi = \partial_t + \Omega_H \partial_\phi$.
...
\begin{align}
    \varepsilon = - k \cdot p = -\mu g_{tt} p^t - g_{t\phi} p^\phi  ~.
    \label{eq2:PenroseE0}
\end{align}
...
Fortunately, the NP formalism provides an alternative method of studying perturbation.
Results as a natural introduction of spinor techniques into GR, following the introduction of a null complex tetrad basis. The full set of NP equations is a system of first-order coupled equations linking the tetrad, the resultant connection (spin coefficients) and projections of the field tensors.  
