...
\end{align}
where $a$ is a parameter, $M$ is the Komar mass and $\rho^2 = r^2 + a^2 \cos^2\theta$. Naturally the time Killing vector is $\partial_v$ and $\partial_\chi$ is the axial field, which implies that $J = a M$.
...
\begin{align}
    \begin{split}
        ds^2 = &- \left(\dd t - \frac{2 M r}{\rho^2} \right) \dd t^2 - 2 a \sin^2\theta \frac{(r^2+a^2-\Delta)}{\rho^2} \dd t \dd \phi \\
...
\begin{align}
    v = t + r_* ~, \qquad \chi = \phi + \int \frac{a}{\Delta} \dd r ~,
    \label{eq2:InEFtoBL}
\end{align}
...
Now, to study the possible horizons of Kerr BH, the one-form $n = \dd r$, defines normal vectors to constant radial hypersurfaces.
...
Due to this and some other unphysical features (for example, closed timelike curves and instabilities under perturbations), we need only to focus on the region outside the event horizon $r>r_{+}$, since only information on that region is physically reachable from a stationary observer point of view.
...
First, consider the quantities $\xi_\alpha u^\alpha$, where $u^\alpha$ is the four-velocity and $\xi^\alpha$ is a Killing field. 
Being aware of the geodesic equation, $u^\beta \nabla_\beta u^\alpha = 0$, it is easy to show that this quantity is conserved along geodesics, \emph{i.e.}
\begin{align}
    u^\beta \nabla_\beta ( \xi_\alpha u^\alpha ) = u^\alpha u^\beta \nabla_\beta \xi_\alpha = \frac{u^\alpha u^\beta }{2} ( \nabla_\alpha \xi_\beta + \nabla_\beta \xi_\alpha ) = 0 ~,
...
\begin{align}
    -\varepsilon &=  k^\beta g_{\alpha\beta} \frac{\dd x^\alpha}{\dd \tau} ~, \qquad \ell = m^\beta g_{\alpha\beta} \frac{\dd x^\alpha}{\dd \tau} ~,
...
\begin{align}
    \dot{t} &= \frac{1}{\Delta} \left[ (r^2+a^2 +\frac{2 M a^2}{r})\varepsilon - \frac{2 M a}{r} \ell \right] ~, \label{eq2:geodesicT} \\
    \dot{\phi} &= \frac{1}{\Delta} \left[ \frac{2 M a}{r} \varepsilon +\left( 1- \frac{2 M}{r} \right) \ell \right]  ~,
    \label{eq2:geodesicPhi}
\end{align}
specified for the equatorial plane $\theta=\pi/2$. The final equation for the geodesic is provided by the line element (\ref{eq2:KerrBL}), which becomes also a first order ODE, after the substitution of $\dot{t}$ and $\dot{\phi}$. Consider now a zero angular momentum observer (ZAMO) infalling radially, with $\ell=0$, then we can get the angular velocity $\Omega$, as measured at infinity
\begin{align}
    \Omega = \frac{\dot{\phi}}{\dot{t}} = - \frac{g_{t\phi}}{g_{\phi\phi}} = \frac{2 a M}{r^3 + a^2 (2 M+r)} ~.
    \label{eq2:angMomentumZAMO}
\end{align}
Asymptotically we obtain $\Omega\to0$, coincident with a static observer measurement, but for a finite distance, observers are forced to co-rotate with the BH. 
...
\end{align}
The field Killing vector $\xi = k + \Omega_H m$ is a special linear combination that is also a null vector normal to the event horizon, $\xi^\flat=\xi_\alpha \dd x^\alpha \propto \dd r$ and $\xi^2 = 0$ at $r=r_+$, which can be written in BL coordinates as $\xi = \partial_t + \Omega_H \partial_\phi$.
Hence, on the outer horizon the integral curves of this vector obey $\xi^\alpha \partial_\alpha (\phi - \Omega_H t) = 0$, resulting in $\phi \propto \Omega_H t$.
...
One of the main characteristic that distinguishes Kerr BH from other spherical solutions is the existence of a \emph{ergoregion} which allows for energy extraction from th BH an therefore superradiance.
...
Notice that a static observer moves in a timelike curve with $(r,\theta,\phi)$ constant, \emph{i.e.} with tangent vector proportional to $k^\mu$, therefore such observer cannot exist inside because the time Killing vector becomes spacelike, otherwise it would violate causality. We can see that $u^2=g_{\alpha\beta}u^\alpha u^\beta = g_{tt} (u^t)^2 + 2 g_{t\phi} u^t u^\phi +  g_{\phi\phi} (u^\phi)^2 < 0$ only occurs when $g_{t\phi} u^\phi < 0$, as all other components are positive. Inside the ergoregion, $g_{t\phi}<0$, therefore all observers are forced to rotate in the same direction as the BH.
...
\begin{figure}[h]
    ...
    \begin{subfigure}[c]{0.4\textwidth}
        ...
        \caption{~$a=0.9 M$, ~$\ell<0$}
    \end{subfigure}
    \caption{Illustration of the Schwarzschild ({\footnotesize\textsc{A,C}}) and Kerr ({\footnotesize\textsc{B,D}}) null equatorial infalling geodesics given by Eqs.~\eref{eq2:geodesicT} and~\eref{eq2:geodesicPhi}, for $r(0)=20 M$, with emphasis on $\ell\ne0$. Even starting with opposite angular momentum, the Kerr  geodesic ({\footnotesize\textsc{D}}) is forced to co-rotate with the BH once crossed the ergoregion (dashed).}
    \label{fig2:geodesics}
\end{figure}

Despite BHs being always thought as ``perfect absorvers'' due to casual separation of the horizon, but the ergoregion is a region that allows energy extraction from the BH, through the Penrose process, an intrinsic feature of rotating BHs.
Much like spontaneous pair creation and amplification at discontinuities are related but distinct effects, the Penrose process allows for a better understating of superradiance and why it is relevance in GR. Considering a particle with rest mass $\mu$ and four-momentum $p^\alpha = \mu u^\alpha$, we may identify the constant of motion 
\begin{align}
    \varepsilon = - k_\alpha p^\alpha = -\mu g_{tt} p^t - g_{t\phi} p^\phi  ~.
    \label{eq2:PenroseE0}
\end{align}
as it's energy, as measured by a stationary observer at infinity, due to relations~\eref{eq2:geodesicConsts}. As shown above the Killing vector is asymptotically timelike but is spacelike inside the ergoregion, thus $g_{tt}>0$. Since the geodesic is future-directed, $p^0 = \mu u^0 >0$ and the energy beyond the ergosurface needs not to be positive. Suppose, by any means, that such particle manages to decay inside the ergoregion into two other, with momenta $p_1$ and $p_2$, such that $p=p_1+p_2$. Contracting with $k$, implies that $\varepsilon = \varepsilon_1+\varepsilon_2$. Supposing that the first of the particles has negative energy, $\epsilon_1<0$, then 
\begin{align}
    \varepsilon_2 = \varepsilon + |\varepsilon_1| > \varepsilon ~.
    \label{eq2:PenroseE0}
\end{align}
