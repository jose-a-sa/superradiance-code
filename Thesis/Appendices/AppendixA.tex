...
\end{align}
due to extra terms resultant of the tetrad derivatives. Tetrad decompositions, $A^a$, are scalars and must not be mistaken as vector fields such as $(e_a)^\mu$ or $A^\mu$. These terms can be written using the spin connection
\begin{align}
...
Obviously, the spin connection replaces the Christoper symbols, $\Gamma^\rho_{\mu\nu}$, in the tetrad formalism. 
We generally need $\tfrac{1}{2} d^2(d+1)$ computations to fully define the latter, while the spin connection has $\tfrac{1}{2} d^2(d-1)$ independent components. For $d=4$, the use of the spin connection implies $16$ components less to work with.